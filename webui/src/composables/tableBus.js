...
// - consumeBatchHint：批量条首次出现提示的会话级一次性标记（模块级状态，
//   ServerTable 卸载重挂载不重置——「只首次会话内提示」）
...
let batchHintShown = false
/** 首次调用返回 true（提示该显示），此后会话内恒 false */
export function consumeBatchHint() {
  if (batchHintShown) return false
  batchHintShown = true
  return true
}
