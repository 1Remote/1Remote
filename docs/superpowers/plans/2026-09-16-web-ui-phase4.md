...
- 关键已知事实：`SessionControlService` partial 五文件，`_connectionId2Hosts` ConcurrentDictionary<connectionId, HostBase>（连接字典）——ServerDto.connectionState 从中派生"该服务器是否有活动会话"（含 Host 内 ProtocolServer.Id 匹配）；导入器在 `ServerPageViewModelBase.cs:210-470`（CmdImport 区域，与 VM 耦合部分需剥离复用其核心）；`PRemoteMTransferHelper`/`mRemoteNGImporter`/`RdpConfig` 均为可静态调用的 Utils；树展开持久化键 SEP=`" ]=+=+=+=>[ "`（SideTree 已有 fullKey）；vuedraggable 4.1.0 已装。
- 批量连接阈值：>5 台弹确认（spec 记录项）。键盘：E=编辑（编辑器已有）、Del=删除（已有 onDelete，需行级键盘接线）、Ctrl+D=复制。
...
- DtoMapper.FromServer 增加可选参 `bool connected`（默认 false）；/api/servers、/api/search 端点在 lock(gd) 快照后，从 `IoC.Get<SessionControlService>()` 的连接字典（读其公共属性或新增快照方法——**注意线程安全**：字典本身 Concurrent，遍历安全；不要在 Host 上调用方法）匹配 `vm.Server.Id`。状态值："connected"（有活动会话）/"disconnected"。
- 前端：StatusDot 已有 connected 渲染分支——自动点亮；SSE reload 已刷列表（断开后 GlobalData.ReloadAll 触发？**验证**：Host 关闭→OnClosed→是否触发 ReloadAll/OnReloadAll——若不触发，SessionControlService 的事件链补一个 SSE 通知：订阅 HostBase.OnClosed→gd.ReloadAll 或直接触发 WebUi 的 reload 推送。研究后选择最小改动）。
...
- `POST /api/servers/import` multipart/form-data：file + ds + kind 自动嗅探（扩展名+内容：.csv→mRemoteNGImporter、.json→1Remote 导出格式（WPF CmdExportJson 的格式——读其导出代码对齐）、.rdp→RdpConfig.FromRdpFile+Windows 凭据管理器读取（TERMSRV/<host> Credential.Load，服务器上有）、.db→PRemoteMTransferHelper 旧库）。返回 {added, skipped, errors[]}。
- `GET /api/servers/export?ids=a,b,c&ds=` → JSON 文件下载（Content-Disposition；格式=WPF 导出格式）。
...
- 树拖拽（Plan 1 简化语义落地）：vuedraggable 或原生 HTML5 DnD——节点拖动：悬停文件夹>400ms=移入（高亮容器）；上下边缘 25%=前/后插（指示线）；跨数据源/后代/只读=禁止光标。落点计算 TreeNodes 重排→逐台 PUT config（TreeNodes 字段）或新增批量端点 `POST /api/servers/batch-nodes {moves:[{id,treeNodes}]}`（复用 batch patch 的 treeNodes——**Plan 2 batch 明确拒绝了 treeNodes**→新增专用小端点更干净：直接 PUT each config 的 TreeNodes 字段即可，N 通常小，循环 PUT 可接受）。持久化 CustomNodeOrder 同级排序可选（读 LocalityTreeViewService.CustomNodeOrder 键=节点 Id——后端帮手：`POST /api/ui-state/tree-order` {parentId→[(nodeId,order)]}——简化：只持久化 TreeNodes 移动，同级自定义顺序暂不落地 Web（WPF 存的 order 继续透传尊重），记录简化）。
...
- 列宽（GridSplitter 语义简化为可拖列边+双击重置）与列显隐（"列"下拉菜单已有占位→实现）→ localStorage '1r-cols'（key=列名→{width,hidden}）+ API 镜像可选（仅 localStorage 即可，记录）。
